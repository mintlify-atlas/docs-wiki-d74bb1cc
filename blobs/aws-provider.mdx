...
| File | Type | Role |
| --- | --- | --- |
| `AbpBlobStoringAwsModule.cs` | `AbpModule` | Depends on `AbpBlobStoringModule` and `AbpCachingModule`. |
...
| `UseTemporaryCredentials` | `false` | Use STS `GetSessionToken` to mint short-lived session credentials. |
...
STS-based flows use the access key + secret (or a profile-derived `AWSCredentials`) to call `GetSessionTokenAsync` (for `UseTemporaryCredentials`) or `GetFederationTokenAsync` (for `UseTemporaryFederatedCredentials`) on `AmazonSecurityTokenServiceClient`, encrypt the resulting `AccessKeyId`, `SecretAccessKey`, and `SessionToken` via `IStringEncryptionService`, and cache the ciphertext in `IDistributedCache<AwsTemporaryCredentialsCacheItem>` keyed by `TemporaryCredentialsCacheKey`. Subsequent calls within the credential's validity window read from the cache and decrypt back into a `SessionAWSCredentials` instance. Cache TTL is `DurationSeconds - 10` so that a stale entry never outlasts the credentials themselves.
...
flowchart TB
    ...
    F -->|UseTemporaryCredentials| S["GetSessionTokenAsync + IDistributedCache"]
