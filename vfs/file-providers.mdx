...
- **Internal** providers — `AbpEmbeddedFileProvider`, `PhysicalFileProvider`, `ManifestEmbeddedFileProvider`, the `EnumerableDirectoryContents`. They live inside a `VirtualFileSetInfo` and are reached only through the composite.
