...
The defaults match the JWT claim names used by both OpenIddict and
IdentityServer:

| `AbpClaimTypes` property | Default value |
| --- | --- |
| `UserId` | `sub` (OpenIddict) or `nameidentifier` (IdentityServer) |
...
The JwtBearer handler maps incoming JWT claims to these names through the
JwtBearer handler's `ClaimsIssuer` and via ABP's `AbpClaimsMapMiddleware`
(part of `Volo.Abp.AspNetCore`). When the OpenIddict module is loaded into
the same process as JwtBearer (rare in production but common in samples)
the OpenIddict module rewrites `AbpClaimTypes` to OpenIddict's standard
names &mdash; see [OpenIddict server](/auth/openiddict-server) for the
full mapping.
...
server's `Authority + /api/account/dynamic-claims` endpoint (the value
stitched together by `AbpJwtBearerExtensions`) and merges the returned
claim list into the current `ClaimsPrincipal`. The cache class
...
sequenceDiagram
    ...
    alt cache hit
        ...
        DC->>AS: GET /api/account/dynamic-claims<br/>Authorization: Bearer ...
