...
│   ├── ng-packs/
│   │   ├── lerna.version.json            # version: 7.2.3, packs: packages/*
...
`ng-packs/` contains two Lerna-shaped manifests at rest, neither named `lerna.json`:

```json title="npm/ng-packs/lerna.version.json"
{
  "version": "7.2.3",
  "packages": [
    "packages/*"
...
`lerna.version.json` is read by `publish.ts` only to remember the previous version (for rollback) — it is *not* what Lerna consumes at publish time. `lerna.publish.json` is the manifest that points at the build output (`dist/packages/*`). During publish, `scripts/publish.ts` *renames* `lerna.publish.json` to the canonical `lerna.json` so Lerna's `exec` step sees the dist artifacts, then renames it back when done:
