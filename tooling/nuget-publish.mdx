...
- Every transitive dependency must be resolvable from nuget.org. The 164 `<PackageVersion>` entries in `Directory.Packages.props` (see [tooling/directory-build-props](/tooling/directory-build-props)) are all public packages.
...
A PR changes `<Version>8.0.1</Version>` to `<Version>8.0.2</Version>`. Note that `build-and-test.yml`'s `paths:` filter watches `Directory.Build.props` and `Directory.Packages.props` but *not* `common.props`, so a version-only bump does not by itself rerun CI — but C# changes are typically batched in the same release branch, which does retrigger the workflow. The PR merges into `dev`.
