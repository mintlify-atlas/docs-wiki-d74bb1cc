...
| `Args/CommandLineArgs.cs` | class | Holds `Command`, `Target`, and an `AbpCommandLineOptions` bag for one parsed invocation. |
...
- The `prompt` REPL splits its input line on space (`promptInput.Split(" ")`) and calls `Parse(string[])` — no quote handling.
- The `batch` file runner reads each non-comment line and calls `Parse(string)`, which tokenises with quote awareness through `GetArgsArrayFromLine` before delegating to `Parse(string[])`.
