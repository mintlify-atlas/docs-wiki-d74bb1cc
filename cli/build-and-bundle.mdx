...
| `Bundling/BundlingMode.cs` | enum | `BundleAndMinify`, `Bundle`, `BundleOnly` (placeholder list mode). |
...
flowchart TD
  ...
  EG -->|yes| EH[StyleBundler.Bundle, ScriptBundler.Bundle\n→ global.css, global.js]
...
| `BundleOnly` (`else` branch) | `GenerateStyleDefinitions` / `GenerateScriptDefinitions` emit one `<link>` / `<script>` per contributor file. No concatenation, no `global.css`. Useful for debugging. |
