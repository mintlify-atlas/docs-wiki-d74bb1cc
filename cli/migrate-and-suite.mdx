...
| `Version/PackageVersionCheckerService.cs` | service | Used by `SuiteCommand` to discover the latest preview/RC of the Suite package. |
...
`AbpCliOptions.AlwaysHideExternalCommandOutput` makes `CmdHelper` redirect stdout/stderr to `Stream.Null` regardless of which method is used.
