...
`JsonSerializerOptions.Converters` is searched in insertion order and the first converter whose `CanConvert(type)` returns `true` wins. Because the ABP module registers its converters during boot, *your* converters (added later from your host module's `Configure<AbpSystemTextJsonSerializerOptions>`) end up at the tail of the list — so for an enum, `bool`, or `Guid`, the ABP defaults are reached first. To *replace* an ABP default, either insert your converter at the front of the list (`options.JsonSerializerOptions.Converters.Insert(0, new MyBoolConverter())`) or remove the ABP one. For `DateTime`, remove the modifier instead of competing with the global converter list:
