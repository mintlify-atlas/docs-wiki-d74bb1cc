...
For multi-module databases set a per-module `MigrationsHistoryTable` name so each EF migration assembly records its history independently:
...
| Case-sensitive `WHERE` mismatches | MySQL collation defaults to case-sensitive `utf8mb4_bin` | Explicitly set `utf8mb4_general_ci` on the database/column |
