...
<Note>
`[IgnoreMultiTenancy]` is declared on the `IBackgroundJobsMongoDbContext` interface; the concrete class above inherits the attribute through interface resolution. The EF Core context, by contrast, repeats the attribute on the class — both shapes produce the same host‑only behaviour.
</Note>
