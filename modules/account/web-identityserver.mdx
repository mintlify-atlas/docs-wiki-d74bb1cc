...
```csharp
[DependsOn(
    typeof(AbpAccountWebIdentityServerModule),     // brings AbpAccountWebModule + AbpIdentityServerDomainModule
    typeof(AbpAccountHttpApiModule),
    typeof(AbpAccountApplicationModule),
    typeof(AbpIdentityServerEntityFrameworkCoreModule), // or MongoDB variant
...
`AbpAccountWebModule` and `AbpIdentityServerDomainModule` are pulled in
transitively. Do not also reference `AbpAccountWebOpenIddictModule` — only
one can win.

In `OnApplicationInitialization` the host calls the IDS4
`app.UseIdentityServer()` middleware **before** `app.UseAuthorization()`
so the IDS4 endpoints (`/connect/authorize`, `/connect/token`, etc.) see
the request first. The IDS4 services themselves are registered by your
host (typically via `services.AddIdentityServer().AddAbpIdentityServer()`,
the extension provided by `Volo.Abp.IdentityServer.Domain`).
