...
## How Identity plugs in (reference example)

To make the abstract pattern concrete, here's the inheritance chain in Identity:

```csharp
// In Volo.Abp.Identity.EntityFrameworkCore
public class EfCoreIdentityUserRepository
    : EfCoreUserRepositoryBase<IIdentityDbContext, IdentityUser>,
      IIdentityUserRepository
{
    public EfCoreIdentityUserRepository(IDbContextProvider<IIdentityDbContext> dbContextProvider)
        : base(dbContextProvider) { }

    // Override FindByUserNameAsync to use NormalizedUserName for case-insensitive lookups
    // Add Identity-specific queries (GetRolesAsync, GetClaimsAsync, …)
}
```
...
classDiagram
    ...
    IUserRepository_TUser_ <|-- IIdentityUserRepository
...
The Identity module also has a `MongoIdentityUserRepository` extending `MongoUserRepositoryBase<IIdentityMongoDbContext, IdentityUser>` with the exact same shape. That's why switching Identity from EF to Mongo is a *package swap*, not a code change.
