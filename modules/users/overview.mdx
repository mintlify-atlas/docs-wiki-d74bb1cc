...
And `EfCoreUserRepositoryBase<TDbContext, TUser>` / `MongoUserRepositoryBase<TDbContext, TUser>` provide turn‑key implementations of `IUserRepository<TUser>` for *non‑Identity* aggregates. In the OSS source tree the actual subclasses are `EfCoreBlogUserRepository : EfCoreUserRepositoryBase<IBloggingDbContext, BlogUser>` and `EfCoreCmsUserRepository : EfCoreUserRepositoryBase<ICmsKitDbContext, CmsUser>` — both ride the same generics.
...
classDiagram
    ...
    class EfCoreBlogUserRepository {
        from Volo.Blogging.EntityFrameworkCore
    }
    class CmsUser {
        from Volo.CmsKit
    }
    class EfCoreCmsUserRepository {
        from Volo.CmsKit.EntityFrameworkCore
    }
    ...
    EfCoreUserRepositoryBase <|-- EfCoreCmsUserRepository
    EfCoreBlogUserRepository --> BlogUser
    EfCoreCmsUserRepository --> CmsUser
```

<Note>
Identity does **not** use `EfCoreUserRepositoryBase`. Its `EfCoreIdentityUserRepository` extends `EfCoreRepository<IIdentityDbContext, IdentityUser, Guid>` directly and implements `IIdentityUserRepository : IBasicRepository<IdentityUser, Guid>` — Identity needs claims/roles/logins joins that the generic base doesn't model. The shared bit between Identity and the generic Users module is the `IUser` contract on `IdentityUser` and the `ConfigureAbpUser<TUser>` model‑builder extension; the repository inheritance is independent.
</Note>
