...
These two records live in `Domain.Shared` so any client can reference them without taking the management UI:
...
The integration service is what `HttpClientPermissionFinder` calls under the hood — that is the bridge that turns a remote host's `IPermissionFinder` into a REST call to the management host.
