...
routes are `/api/docs/documents/...` and `/api/docs/projects/...`.
...
The Markdown → HTML pipeline has two layers. `IMarkdownConverter` (default
`MarkDigMarkdownConverter` in `Volo.Docs.Web/Markdown/`) wraps Markdig and
does the raw Markdown-to-HTML conversion. `IDocumentToHtmlConverter`
(default `MarkdownDocumentToHtmlConverter` in `Volo.Docs.Web/Markdown/`)
adds the document-aware post-processing — link rewriting, anchor
normalisation, and Scriban-based partial expansion via
`ScribanDocumentSectionRenderer` (under `Volo.Docs.Web/HtmlConverting/`).
The factory `IDocumentToHtmlConverterFactory` (concrete
`DocumentToHtmlConverterFactory`) picks the right `IDocumentToHtmlConverter`
by `Document.Format`, looking it up in
`DocumentToHtmlConverterOptions.Converters` registered by `DocsWebModule`:

```csharp Volo.Docs.Web/DocsWebModule.cs
Configure<DocumentToHtmlConverterOptions>(options =>
{
    options.Converters[MarkdownDocumentToHtmlConverter.Type]
        = typeof(MarkdownDocumentToHtmlConverter);
});
```

### Reader page routes

`DocsUiOptions.RoutePrefix` defaults to `"documents"`. With that prefix
the conventions registered in `DocsWebModule.ConfigureServices` produce:

| Route family | Razor page | Purpose |
| --- | --- | --- |
| `/documents/{projectName}` | `/Documents/Project/Index` | Project landing (default language + version) |
| `/documents/{languageCode}/{projectName}` | `/Documents/Project/Index` | Project landing in a specific language |
| `/documents/{languageCode}/{projectName}/{version}/{*documentName}` | `/Documents/Project/Index` | Versioned, language-scoped document page (catch-all path) |
| `/documents/search/{languageCode}/{projectName}/{version}` | `/Documents/Search` | Project-scoped search results |
| `/documents` | `/Documents/Index` | Projects index |
...
Configure<RazorPagesOptions>(options =>
{
    ...
    options.Conventions.AddPageRoute("/Documents/Project/Index",
        routePrefix + "{projectName}");
    options.Conventions.AddPageRoute("/Documents/Project/Index",
        routePrefix + "{languageCode}/{projectName}");
    options.Conventions.AddPageRoute("/Documents/Project/Index",
        routePrefix + "{languageCode}/{projectName}/{version}/{*documentName}");
    options.Conventions.AddPageRoute("/Documents/Search",
        routePrefix + "search/{languageCode}/{projectName}/{version}");
});
```
...
* [CMS Kit](/modules/cms-kit/overview) — alternative content module focused on
