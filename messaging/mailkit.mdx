...
`MailKitSmtpEmailSender` is registered explicitly via `[Dependency(ServiceLifetime.Transient, ReplaceServices = true)]` (see below), so it supplants the default `SmtpEmailSender` as soon as `AbpMailKitModule` is in the dependency graph. `AbpMailKitOptions` is a plain options class wired through `IOptions<T>` — no DI marker is needed.
