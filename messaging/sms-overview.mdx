...
`NullSmsSender` is `[ITransientDependency, ISingletonDependency]`-decorated and registered via `[Dependency(TryRegister = true)]` (see below), so it only takes effect when no other module has claimed `ISmsSender`.
