...
The module sets up two layers of caching that downstream Razor components see as plain services — `ICachedApplicationConfigurationClient` and `AbpComponentsClaimsCache`. See [Blazor](/blazor) for how these flow into `CascadingAuthenticationState`.
...
See [HTTP client](/http) for the proxy infrastructure and authentication-token plumbing.
...
- [HTTP client](/http) — dynamic proxies, `RemoteServiceOptions`, `AbpHttpClientBuilderOptions`, authentication plumbing.
- [Authorization](/authz) — `ICurrentUser`, claims caches, permission checks behind the proxy calls.
