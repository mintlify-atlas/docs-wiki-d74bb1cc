...
<Steps>
  ...
  <Step title="Wire up authentication">
    For interactive desktop apps, plug in `Microsoft.Identity.Client` or `IdentityModel.OidcClient` and supply an `HttpMessageHandler` that attaches access tokens — see [Authorization](/authz).
  </Step>
...
- [HTTP client](/http) — dynamic proxies and `RemoteServiceOptions` you'll bolt onto the WPF template.
- [Authorization](/authz) — `ICurrentUser`, claims, and how to feed access tokens through the proxy pipeline.
