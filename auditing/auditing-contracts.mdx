...
`IHasDeletionTime` extends `ISoftDelete` — implementing it automatically opts the entity into the soft-delete filter from the [unit of work and data filters](/uow) pipeline.
...
- [Unit of work](/uow) — how `ISoftDelete` (transitively brought in by `IHasDeletionTime`) participates in the data filter pipeline.
