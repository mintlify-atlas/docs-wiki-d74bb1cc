...
- `UnitOfWorkManager.Begin(true)` opens a **new** transactional UoW so persistence is not coupled to the business UoW that produced the data (which may already have been committed or rolled back). See [unit of work](/uow/overview).
...
- [Unit of work](/uow/overview) — the `IUnitOfWorkManager.Begin(true)` call inside `AuditingStore.SaveLogAsync`.
