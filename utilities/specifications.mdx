...
For projects that expose a query DSL over HTTP (think GraphQL-style filter strings or OData `$filter`), `ISpecificationParser` is an extension point. The framework does not ship a default — it is intended for module authors who want to register their own parser per entity type. The contract is intentionally generic; the package leaves the implementation to you.
