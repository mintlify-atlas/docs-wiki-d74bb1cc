...
The native `maui` template targets `.NET MAUI` shells/pages directly (XAML), not MAUI Blazor. MAUI Blazor is offered by the `app` template through the `MyCompanyName.MyProjectName.MauiBlazor` project.
